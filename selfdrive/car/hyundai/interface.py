...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False):  # pylint: disable=dangerous-default-value
    ...
    # hyundai
      ...
      ret.wheelbase = 2.766
      ret.centerToFront = ret.wheelbase * 0.4
    elif candidate in [CAR.SANTA_FE_2022, CAR.SANTA_FE_HEV_2022, CAR.MAXCRUZE]:
    ...
    # kia
      ...
      tire_stiffness_factor = 0.8
    elif candidate in [CAR.K7, CAR.K7_HEV, CAR.K7_2020, CAR.K7_HEV_2020]:
      tire_stiffness_factor = 0.7
  ...
  # 버튼 누름 처리
    for b in ret.buttonEvents:
      # 버튼 눌림 상태에 따라 이벤트 추가do disable on button down
      if b.type == ButtonType.cancel and b.pressed:
        events.add(EventName.buttonCancel)
      if self.CC.longcontrol and not self.CC.scc_live:
        # 가속 및 감속 버튼에서 이벤트 추가do enable on both accel and decel buttons
        ...
        # 감속 버튼에서 이벤트 추가 do enable on decel button only
